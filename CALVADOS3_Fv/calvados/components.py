...
class Protein(Component):
    ...
    def calc_custom_res_restrained(self):
        """ Get bounds for restraints (harmonic). """
        self.custom_restraints = build.get_custom_res_restrained(self.custom_restraints_file,param_threshold=-0.41)
        print(f'Setting custom restraints for {self.name}')
    ...
    def calc_properties(self, pH: float = 7.0, verbose: bool = False, comp_setup: str = 'spiral'):
        ...
        if self.restraint:
            ...
            if self.restraint_type == 'harmonic':
                self.calc_ssdomains()
                if self.custom_restraints_file is not None:
                    self.calc_custom_res_restrained()
                    print('Custom restraints:',self.custom_restraints)

               # print('print',self.ssdomains)
...
#                if self.restraint_type == 'harmonic':
#                    k = self.k_custom[-1]
#                    ss = build.check_ssdomain(self.ssdomains,i,j,req_both=True) ## added last
#                    if  ss:
#                        k = self.k_harmonic
#                        
#                        cr,k_custom_idx = build.check_custom_restraints(self.custom_restraints,i+offset,j+offset)
#                        #print(i+offset,j+offset,cr)
#                        if cr:
#                            k=self.k_custom[k_custom_idx]
#                            #print(f'Custom restraint between {i+1} and {j+1} with k={k} and k_custom_idx={k_custom_idx}') 
#                # go
#                elif self.restraint_type == 'go':
#                    if self.scale[i,j] < min_scale:
#                        continue
...
#                        i+offset, j+offset)
#                #if cr or not cr:
#                    #print(k,i+offset,j+offset,restr_pair)
#                self.restr_pairlist.append(restr_pair)
#                exclusion_map.append([i+offset,j+offset])
#        return exclusion_map

### modified - flexible domain
#    def add_restraints(self, offset, min_scale = 0.1):
#        """ Add restraints. """
#        exclusion_map = [] # for ah, yu etc.
#        for i in range(0,self.nbeads-2):
...
#                    if  ss:
#                        #k = self.k_custom[-1]
#                        if build.check_ssdomain(self.ssdomains,i,j,req_both=True):
#                            k = self.k_harmonic
#                        
#                        cr,k_custom_idx = build.check_custom_restraints(self.custom_restraints,i+offset,j+offset)
#                        if cr:
...
### modified - flexible domain with contacts restraints
    def add_restraints(self, offset, min_scale = 0.1):
        """ Add restraints. """
        exclusion_map = [] # for ah, yu etc.
        for i in range(0,self.nbeads-2):
