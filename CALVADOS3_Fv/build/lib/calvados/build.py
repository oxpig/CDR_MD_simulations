...
def get_custom_res_restrained(custom_restr_file,param_threshold=-1.0):
    custom_restr = []
    df_pairs=pd.read_csv(custom_restr_file,delimiter=',')
    # Group by columns 'i' and 'j', and select the row with the minimum 'parameter' value for each group (further from zero)
    df_pairs['i_input'] = df_pairs['i']
    df_pairs[['i', 'j']] = df_pairs[['i', 'j']].apply(sorted, axis=1, result_type='expand')
    df_pairs_unique = df_pairs.loc[df_pairs.groupby(['i', 'j'])['parameter'].idxmin()]
    
    # Add a new column 'k' based on the value of 'params'
    df_pairs_unique['k'] = df_pairs_unique['parameter'].apply(lambda x: 0 if x < param_threshold else 1)
    # Convert the resulting DataFrame to a list of tuples (i, j, params, k)
    custom_restr = list(df_pairs_unique[['i_input','i', 'j', 'k']].itertuples(index=False, name=None))
...
def check_custom_restraints(custom_restraints,i,j):
    """
    Check if one (req_both == False) or both (req_both == True) of
    the residues are in a structured domain.
    1-based --> input is 0-based
    """
    cr=False
    k_idx=0
    for pair in custom_restraints:
        if ((i+1) in pair[1:-1]) and ((j+1) in pair[1:-1]):
                cr = True
                k_idx=pair[-1]
                #print(pair,pair[1:-1],i,j,k_idx,cr)

    ## mod 13feb - decomment if rigid restraints are used  
    #if cr == False:
    #    for pair in custom_restraints:
    #        if (i+1) == pair[0]: 
    #            cr = True
    #            k_idx=2
    #            #print('sc',pair,0,i,j,k_idx,cr)
